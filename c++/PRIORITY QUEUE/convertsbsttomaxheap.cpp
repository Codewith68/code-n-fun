...
// Convert BST to Max Heap by replacing node values using inorder traversal result
...
void BSTtoMaxHeap(Node* root) {
    ...
    int index = 0;
    convertToMaxHeap(root, inorder, index); // Overwrite nodes with heap values
}
...
int main() {
    ...
    cout << "After Converting to Max Heap: ";
    LevelOrderQueue(root);
